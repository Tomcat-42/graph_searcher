...
from utils.json_file_action import read_json_file
...
def nova_janela():
    ...
    termo = Label(janela2, text="Dados aleatorizados",
                  background="#008", foreground="#fff", font=("X", 15))
    termo.place(x=0, y=10, width=700, height=40)
    grafo_e = Label(janela2, text=dados_grafo, background="#008",
                    foreground="#fff", font=("X", 15))
    ...
    ###############################################
    ###############################################
    texto1 = Label(janela2, text="Estado inicial:",
                   background="#008", foreground="#fff", font=("X", 15))
    ...
    texto2 = Label(janela2, text="Estado final:  ",
                   background="#008", foreground="#fff", font=("X", 15))
    ...
    botaoM = Button(janela2, text="Busca em largura",
                    command=nova_janela, font=("X", 15))
    botaoM.place(x=50, y=450)

    botaoB = Button(janela2, text="Busca com limite superior",
                    command=entrada_grafo, font=("X", 15))
    botaoB.place(x=230, y=450)


def load_json(arquivo):
    data = read_json_file(arquivo)
...
def entrada_grafo():
    ...
    # Texto pedindo para inserir o nome do json a ser carregado
    texto = Label(janela3, text="Selecione o arquivo:",
                  background="#008", foreground="#fff", font=("X", 15))
    ...
    texto2 = Label(janela3, text=text, background="#008",
                   foreground="#fff", font=("X", 15))
    # Deixa o texto centralizado
    texto2['justify'] = 'center'
    texto2.place(x=1, y=150, width=500, height=40)

    # Botão para carregar o arquivo
    botao = Button(janela3, text="Carregar",
                   command=lambda: load_json(arquivo), font=("X", 15))
    botao.place(x=100, y=200)
    # Botão para voltar
    botao2 = Button(janela3, text="Voltar",
                    command=nova_janela, font=("X", 15))
    botao2.place(x=300, y=200)
...
texto = Label(janela, text="Escolha a forma da inserção dos dados:",
              background="#008", foreground="#fff", font=("X", 15))
texto.place(x=1, y=100, width=500, height=40)
